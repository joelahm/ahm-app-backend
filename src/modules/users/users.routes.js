...
router.get(
  "/permissions",
  requireRole("ADMIN"),
  usersController.getPermissionsSettings,
);
router.patch(
  "/permissions",
  requireRole("ADMIN"),
  usersController.patchPermissionsSettings,
);
router.get("/", requireRole("ADMIN"), usersController.listUsers);
router.get(
  "/activity-logs",
  requireRole("ADMIN"),
  usersController.listActivityLogs,
);
router.get(
  "/pending-invitations",
  requireRole("ADMIN"),
...
router.delete(
  "/pending-invitations/:id",
  requirePermission("remove-user"),
  usersController.cancelPendingInvitation,
);
router.post(
  "/pending-invitations/:id/resend",
  requirePermission("add-new-user"),
  usersController.resendPendingInvitation,
);
router.post("/", requirePermission("add-new-user"), usersController.createUser);
router.post(
  "/invite",
  requirePermission("add-new-user"),
  usersController.inviteUsers,
);
router.patch("/:id", requireRole("ADMIN"), usersController.patchUser);
router.patch(
  "/:id/role",
  requirePermission("change-user-role"),
  usersController.patchUserRole,
);
router.patch(
  "/:id/password",
  requireRole("ADMIN"),
