...
function readEnv() {
  ...
  if (!databaseUrl) {
    throw new Error(
      "Missing required database configuration. Set DATABASE_URL or MYSQL_* variables.",
    );
  }
  ...
  return {
    ...
    authCookies: {
      ...
      secure:
        String(
          process.env.COOKIE_SECURE || cookieSecureDefault,
        ).toLowerCase() === "true",
    ...
    superadminUsers: readEmailList(process.env.SUPERADMIN_USERS),
    invite: {
    ...
    integrations: {
      ...
      discord: {
        ...
        defaultChannelId: process.env.DISCORD_DEFAULT_CHANNEL_ID || null,
      },
    },
    scans: {
